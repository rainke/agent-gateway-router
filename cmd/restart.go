...
var restartCmd = newRestartCommand(startDaemon)

func newRestartCommand(startInBackground func() error) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "restart",
		Short: "重启 agr 网关服务",
	...
	cmd.Flags().StringVarP(&configFile, "config", "c", "", "指定 TOML 配置文件路径")
	cmd.Flags().IntVarP(&port, "port", "p", 0, "覆盖配置文件中的监听端口")
	return cmd
}
